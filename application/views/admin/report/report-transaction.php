<?php if (!defined('BASEPATH')) exit('No direct script access allowed');

?>
<div class="col-lg-12">
    <div class="panel panel-default">
        <div class="panel-heading">
<form id="formFilter" method="POST" target="_blank">
<input type="hidden" id="type" name="type" value="transaction">
    <div class="clear-export"></div>
            <table class="col-lg-8 form-table-cms">
                <tbody>

                 <tr>
                    <td><label>Kasir</label></td>
                    <td>   <?php 
                    echo form_dropdown('user_id', $all_cashier, '', 
                        'id="user_id" field-name = "" 
                        class="form-control" autocomplete="on"');
                        ?></td>

                    </tr>
                    <tr>
                    <td><label>Tipe Pesanan</label></td>
                    <td>
                      <select name="order_type" class="form-control">
                        <option value="">Semua Tipe Pesanan</option>
                        <?php foreach($order_types as $key=>$o): ?>
                        <option value="<?php echo $key ?>"><?php echo $o ?></option>
                        <?php endforeach; ?>
                      </select>
                    </td>

                    </tr>
                    <tr>
                    <td><label>Tipe Pembayaran</label></td>
                    <td>
                      <select name="payment_option" class="form-control">
                        <option value="">Semua Tipe Pembayaran</option>
                        <?php foreach($payment_options as $key=>$o): ?>
                        <option value="<?php echo $key ?>"><?php echo $o ?></option>
                        <?php endforeach; ?>
                      </select>
                    </td>

                    </tr>
                    <tr>
                    <td><label>Waktu Mulai</label></td>
                    <td class="col-sm-8">
                            <div class='input-group date ' id='start_date'>
                             <?php echo form_input(array('name' => 'start_date',
                               'id' => 'input_start_date',
                               'type' => 'text',
                               'class' => 'form-control date',
                               'onkeydown'=>'return false',
							   'value'=>date("Y-m-d")." 00:00",

                               )); ?>
                               <span class="input-group-addon">
                                <span class="glyphicon glyphicon-calendar">
                                </span>
                            </span>
                        </div> 
                    </td>
                  
                </tr>
                <tr>
                    <td><label>Waktu Akhir</label></td>
                    <td class="col-sm-8">
                            <div class='input-group date ' id='end_date'>
                             <?php echo form_input(array('name' => 'end_date',
                               'id' => 'input_end_date',
                               'type' => 'text',
                               'class' => 'form-control date',
                               'onkeydown'=>'return false',
							   'value'=>date("Y-m-d")." 23:59",

                               )); ?>
                               <span class="input-group-addon">
                                <span class="glyphicon glyphicon-calendar">
                                </span>
                            </span>
                        </div> 
                    </td>
                    
                    
                </tr>
               
               
                <tr>
                    <td colspan="4" align="right">
                        <!-- <button id="export_xls" class="btn btn-default hide_btn" style="display: none">Export XLS</button> -->
                        <button id="filter_submit" class="btn btn-default"><i class="fa fa-filter" aria-hidden="true"></i> Filter</button>
                        <button id="export_pdf" class="btn btn-success hide_btn" ><i class="fa fa-file-pdf-o" aria-hidden="true"></i> Export PDF</button>
                        
                    </td>
                </tr>
                </tbody>
            </table>
</form>

            <div class="clearfix"></div>
        </div>
        <!-- /.panel-heading -->
        <div class="panel-body">
					<ul class="nav nav-tabs">
						<li class="active"><a href="#summary-tab" data-toggle="tab">Summary</a></li>
						<li><a href="#detail-tab" data-toggle="tab">Detail</a></li>.
					</ul>
					<div class="tab-content">
						<div class="tab-pane fade in active" id="summary-tab" style="padding-top: 20px; position:relative;">
                            <table class="col-lg-6 form-table-cms">
                                <tbody>
                                    <tr>
                                        <td><label>Periode</label></td>
                                        <td><label id="periode"></label></td>
                                    </tr>
                                </tbody>
                            </table>
                            <div class="clearfix"></div>
							<table class="col-lg-6 form-table-cms">
                                <tbody>
                                    <tr>
                                        <td><label>Total Penjualan</label></td>
                                        <td><label id="total_price"></label></td>
                                    </tr>
                                    <tr>
                                        <td><label>Total Tax</label></td>
                                        <td><label id="total_tax"></label></td>
                                    </tr>
                                    <tr>
                                        <td><label>Total Ongkos Kirim</label></td>
                                        <td><label id="total_ongkir"></label></td>
                                    </tr>
                                    <tr>
                                        <td><label>DP IN</label></td>
                                        <td><label id="total_dp_in"></label></td>
                                    </tr>
                                    <tr>
                                        <td><label>Total Penambahan Modal</label></td>
                                        <td><label id="total_balace_cash"></label></td>
                                    </tr>
                                    <tr>
                                        <td><label></label></td>
                                        <td><label></label></td>
                                    </tr>
									<tr>
                                        <td><label style="margin-top: 5px;">----------------------------------------------------</label></td>
                                        <td><label style="margin-top: 5px;"></label></td>
                                    </tr>
                                    <tr>
                                        <td><label>Total Pendapatan</label></td>
                                        <td><label id="total_price"></label></td>
                                    </tr>
                                </tbody>
                            </table>

                            <table class="col-lg-6 form-table-cms">
                                <tbody>
                                    <tr>
                                            <td><label>Total Diskon</label></td>
                                            <td><label id="total_discount"></label></td>
                                    </tr>
                                    <tr>
                                            <td><label>Total Bon Bill</label></td>
                                            <td><label id="total_profit"></label></td>
                                    </tr>
                                    <tr>
                                            <td><label>Total Voucher</label></td>
                                            <td><label id="total_profit"></label></td>
                                    </tr>
                                    <tr>
                                            <td><label>Total Pending Bill</label></td>
                                            <td><label id="total_profit"></label></td>
                                    </tr>
                                    <tr>
                                            <td><label>DP OUT</label></td>
                                            <td><label id="total_profit"></label></td>
                                    </tr>
                                    <tr>
                                            <td><label>Total Kas Kecil</label></td>
                                            <td><label id="total_petty_cash"></label></td>
                                    </tr>
                                    <tr>
                                        <td><label>----------------------------------------------------</label></td>
                                        <td><label></label></td>
                                    </tr>
                                    <tr>
                                        <td><label>Total Pengurangan</label></td>
                                        <td><label></label></td>
                                    </tr>
                                </tbody>
							</table>
                            <div class="clearfix"></div>
							<table class="col-lg-6 form-table-cms">
								<tbody>
                                    <tr>
                                        <td><label></label></td>
                                        <td><label></label></td>
                                    </tr>
                                    <tr>
                                        <td><label>Total Gross (Pendapatan-Pengurangan) </label></td>
                                        <td><label></label></td>
                                    </tr>
                                    <tr>
                                        <td><label>Total HPP</label></td>
                                        <td><label id="total_cogs"></label></td>
                                    </tr>
                                    <tr>
                                            <td><label>Total Nett</label></td>
                                            <td><label id="total_profit"></label></td>
                                    </tr>
                                    <tr>
                                            <td><label>Total Jasa Kurir</label></td>
                                            <td><label id="total_courier_service"></label></td>
                                    </tr>
                                    <tr>
                                            <td><label>Total Transaksi</label></td>
                                            <td><label id="total_transaction"></label></td>
                                    </tr>

                                    <tr>
                                            <td><label>Total Customer</label></td>
                                            <td><label id="total_customer"></label></td>
                                    </tr>

                                    <tr>
                                            <td><label>Total Penjualan Menu</label></td>
                                            <td><label id="total_quantity"></label></td>
                                    </tr>
								  
								</tbody>
							</table>

                            <table class="col-lg-6 form-table-cms">
                                <tbody>
                                    <tr>
                                        <td><label></label></td>
                                        <td><label></label></td>
                                    </tr>                                  
                                    <tr>
                                        <td><label>Jumlah Dinein</label></td>
                                        <td><label id="dinein_count"></label></td>
                                    </tr>
                                    <tr>
                                        <td><label>Total Dinein</label></td>
                                        <td><label id="dinein_total"></label></td>
                                    </tr>
                                    <tr>
                                        <td><label>Jumlah Takeaway</label></td>
                                        <td><label id="takeaway_count"></label></td>
                                    </tr>
                                    <tr>
                                        <td><label>Total Takeaway</label></td>
                                        <td><label id="takeaway_total"></label></td>
                                    </tr>
                                    <tr>
                                        <td><label>Jumlah Delivery</label></td>
                                        <td><label id="delivery_count"></label></td>
                                    </tr>
                                    <tr>
                                        <td><label>Total Delivery</label></td>
                                        <td><label id="delivery_total"></label></td>
                                    </tr> 
                                </tbody>
                            </table>
							<div class="clearfix"></div>
                            <div class="loading-summary">
                                <img src="<?php echo base_url()?>assets/img/loadgif.gif">
                            </div>
						</div>
						<div class="tab-pane fade in" id="detail-tab">
							<table class="table table-striped table-bordered table-hover dt-responsive" cellspacing="0" width="100%" id="table-transaction">
									<thead>
									<tr>
											<th>Tanggal</th>
											<th>Receipt</th>
											<th>Pembelian</th>
											<th>Grand Total</th>
											<th>Jumlah Pelanggan</th>
											<th>Order ID</th>
											<th>Aksi</th>
									</tr>
									</thead>
								 <!--  <tr>
											<th colspan="9" align="center" valign="middle"><?php echo $this->lang->line('empty_data'); ?></th>
									</tr> -->
							</table>
						</div>
            <input type="hidden" id="dataProcessUrl" value="<?php echo $data_url; ?>"/>

            <!-- /.table-responsive -->
        </div>
        <!-- /.panel-body -->
    </div>
    <!-- /.panel -->
</div>
<input type="hidden" id="report_type" value="transaction"/>
<!-- /.col-lg-12 -->

<!-- /.panel-hidden -->
<div class="panel-hidden" style="display:none">

    <h1 class="page-header"><?php echo $this->lang->line('sales_report_title'); ?></h1>
    <table class="col-lg-12 form-table-cms" style="width:100%;">
        <tbody>
        <tr>
            <td><label><?php echo $this->lang->line('column_start_date'); ?></label></td>
            <td>:</td>
            <td><label id='start_date_hidden'></label></td>
            <td><label> <?php echo $this->lang->line('column_store'); ?></label></td>
            <td>:</td>
            <td><label id='store_hidden'></label></td>
        </tr>
        <tr>
            <td><label><?php echo $this->lang->line('column_end_date'); ?></label></td>
            <td>:</td>
            <td><label id='end_date_hidden'></label></td>
            <td><label><?php echo $this->lang->line('column_outlet'); ?></label></td>
            <td>:</td>
            <td><label id='outlet_hidden'></label></td>
        </tr>
        <tr>
            <td><label><?php echo $this->lang->line('column_date_month'); ?></label></td>
            <td>:</td>
            <td><label id='month_hidden'></label></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td><label><?php echo $this->lang->line('column_date_year'); ?></label></td>
            <td>:</td>
            <td><label id='year_hidden'></label></td>
            <td></td>
            <td></td>
        </tr>

         <tr>
            <td><label>payment</label></td>
            <td>:</td>
            <td><label id='payment_hidden'></label></td>
            <td></td>
            <td></td>
        </tr>

        </tbody>
    </table>

    <table id="table-sales-hidden" class="sales-table" style="border-collapse: collapse;width:100%;">
        <thead class="custom-hidden-table">
        </thead>
    </table>

    <!-- /.table-responsive -->
</div>
<!-- /.panel-hidden -->