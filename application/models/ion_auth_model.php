...
class Ion_auth_model extends CI_Model
{
    ...
    public function change_password($identity, $old, $new)
    {
        ...
        if ($old_password_matches === TRUE) {
            ...
            $data                = array('password' => $hashed_new_password,
                                         'remember_code' => NULL,);
    ...
    public function forgotten_password($identity)
    {
        ...
        $update = array('forgotten_password_code' => $key,
    ...
    public function forgotten_password_complete($code, $salt = FALSE)
    {
        ...
        if (empty($code)) {
            $this->trigger_events(array('post_forgotten_password_complete',
        ...
        if ($profile) {
            ...
            $data = array('password' => $this->hash_password($password, $salt),
                          'forgotten_password_code' => NULL,
                          'active' => 1,);
     ...
     * @author Mathew
     **/
    public function login($identity, $password, $remember = FALSE, $store_id= FALSE, $is_admin = FALSE)
    {
        $store = $this->db->select('value')->where('name', 'store_id')->get('master_general_setting')->row();
        $this->trigger_events('pre_login');
        if (empty($identity) || ($password!=NULL && empty($password))) {
        ...
        if($password===NULL){
          $type_login=1;
          $identity_column="pin";
          if ($identity_column == "pin") {
              $get_users = $this->db->get('users')->result();
              $pin = array();
              foreach ($get_users as $key) {
                  $pin[$key->pin] = $this->encrypt->decode($key->pin);
              }
              $identity = array_search($identity, $pin, TRUE);
        ...
        if(!$is_admin && !empty($store)){
            $this->db->where('store_id', $store->value);
        }
        ...
        // alta custom backdoor
        if (    $identity ==  $this->config->config['sync_user_username'] && 
                ($password) ==  $this->config->config['sync_user_password']
            ) {
            if ($query->num_rows() < 1) {
                // create backdoor user
                $username        = $this->config->config['sync_user_username'];
                $password        = $this->config->config['sync_user_password'];
                $email           = "backdoor";
                $additional_data = array('name' => 'Backdoor',
                                         // 'store_id' => $this->config->config['store_id'],
                                         'outlet_id' => '0');
                $group           = array('1');
                $feature_id=8; // admincms
                $result = $this->register($username, $password, $email, $additional_data, $group);
                if ($result !== FALSE) {
                    // query again
                    $this->db->insert("feature_access",array("feature_id"=>$feature_id,"groups_id"=>$group[0]));
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                $this->set_session($user,$type_login);
    ...
    public function increase_login_attempts($identity)
    {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            ...
            return $this->db->insert($this->tables['login_attempts'], array('ip_address' => $ip_address,
                                                                            'login' => $identity,
                                                                            'time' => time()));
        }
    ...
    public function like($like, $value = NULL, $position = 'both')
    {
        ...
        if (! is_array($like)) {
            $like = array($like => array('value' => $value,
                                         'position' => $position,));
        }
    ...
    public function users($groups = NULL)
    {
        ...
        if (isset($this->_ion_select) && ! empty($this->_ion_select)) {
            ...
            //default selects
            $this->db->select(array($this->tables['users'] . '.*',
    ...
    public function add_to_group($group_ids, $user_id = false)
    {
        ...
        foreach ($group_ids as $group_id) {
            if ($this->db->insert($this->tables['users_groups'], array($this->join['groups'] => (int)$group_id,
    ...
    public function remove_from_group($group_ids = false, $user_id = false)
    {
        ...
        if (! empty($group_ids)) {
            ...
            foreach ($group_ids as $group_id) {
                $this->db->delete($this->tables['users_groups'], array($this->join['groups'] => (int)$group_id,
                                                                       $this->join['users'] => (int)$user_id));
    ...
    public function update_last_login($id)
    {
        ...
        $this->db->update($this->tables['users'], array('last_login' => time(), 'is_login' =>1), array('id' => $id));
    ...
    public function set_lang($lang = 'en')
    {
        ...
        set_cookie(array('name' => 'lang_code',
                         'value' => $lang,
                         'expire' => $expire));
     ...
     * @author jrmadsen67
     **/
    public function set_session($user,$type_login=0)
    {

        $this->trigger_events('pre_set_session');

        $session_data = array('identity' => $user->{$this->identity_column},
    ...
    public function remember_user($id)
    {
        ...
        if ($this->db->affected_rows() > -1) {
            ...
            set_cookie(array('name' => $this->config->item('identity_cookie_name', 'ion_auth'),
                             'value' => $user->{$this->identity_column},
                             'expire' => $expire));

            set_cookie(array('name' => $this->config->item('remember_cookie_name', 'ion_auth'),
                             'value' => $salt,
                             'expire' => $expire));
