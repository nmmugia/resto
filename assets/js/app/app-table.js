...
define([
    ...
    return {
        ...
        initSocketIO: function() {
            ...
            AppTable.socket.on('connected', function(data) {
                ...
                AppTable.socket.on('sm_notify_cooking_status', function(data) {
                    ...
                    $(".list_order[data-order_id='" + data.order_id + "']").find("td[data-order_menu_id='" + data.order_menu_id + "']")
						.attr("data-cooking-status", data.cooking_status)
						.attr("data-cooking_status", data.cooking_status);
                ...
                AppTable.socket.on('sm_set_table_used', function(data) {
                    var tableId = data.table_id;
                    var tableUsed = data.is_used;
                    if (tableUsed == false) {
                        ...
                        request.done(function(msg) {
                            if (msg != '') {
                                ...
                                if (parsedObject.status === true) {
                                    ...
                                    AppTable.tableParent.find(".dine-in-order").show();
                                    ...
                                    AppTable.socket.emit('cm_remove_table_used', {table_id: tableId});

                                    AppTable.socket.emit('cm_notify_change_table', {
                                        number_guest    : parsedObject.data.number_guest,
                                        table_name      : parsedObject.data.table_name,
                                        table_id        : parsedObject.data.table_id,
                                        order_id        : parsedObject.data.order_id
                                    });
        ...
        checkAllReady: function(order_id) {
            if (AppTable.diningType == 3) {
                var count_new = 0;
                var count_ready = 0;
                $(".bill-table tbody tr").each(function() {
                    if (parseInt($(this).find("td:eq(9)").html()) == 0) count_new++;
                    if (parseInt($(this).find("td:eq(9)").html()) != 3) count_ready++;
                });
                ...
                if (count_ready == 0 && count_new == 0) {
                    $("#btn-close-order").show();
                } else {
                    $("#btn-close-order").hide();
                }
        ...
        postToReady: function(customer_name, customer_phone) {
			var data = {};
            if ($("#is_take_away").length > 0) {
				data.order_id = $('#order_id').val();
				data.customer_name = customer_name;
				data.customer_phone = customer_phone;
                var url = AppTable.baseUrl + "cashier/process_takeaway";
            }

            if ($("#is_delivery").length > 0) {
				...
				data.delivery_cost_id = $('#delivery_cost_id').val();
				data.delivery_cost = delivery_cost;
				
                var url = AppTable.baseUrl + "cashier/process_delivery";
            }

            if ($("#is_dine_in").length > 0) {
				data.order_id = $('#order_id').val();
                var url = AppTable.baseUrl + "table/process_order";
            }
            ...
            request.done(function(msg) {
                ...
                if (AppTable.useKitchen == 0 && AppTable.useRoleChecker == 1) {
                    AppTable.socket.emit('cm_notify_checker', {});
                }
				
                setTimeout(function() {
					window.location.href = $(".post_to_ready").attr("href");
                    // window.location = parsedObject.url_redir;
                }, 100);

                // window.location.replace(parsedObject.url_redir);
        ...
        initUIEvent: function() {
            ...
            $(".delete_order_all").on('click', function(e) {
				var object = $(this);
				if(!$(this).attr("skip_confirm")){
					AppTable.confirm('Anda yakin ingin hapus order?', deleteOrder);
					e.preventDefault();
				} else {
					deleteOrder();
				}
                ...
                function deleteOrder() {
                    user_confirmation = object.attr("feature_confirmation");
            ...
            $(".post_to_ready").on('click', function(e) {
                ...
                var count_ord = $('.bill-table > tbody  > tr').length;
                if ($("#is_take_away").length > 0 || $("#is_delivery").length > 0) {
                    if (customer_name == '') {
                ...
                if (user_confirmation != "") {
					...
					AppTable.postToReady(customer_name, customer_phone);
					return false;
				}
            });
            ...
            $(document).on('click', '.dine-in-order, .table-list-text ', function(e) {
                ...
                var isMerged = $(this).has(".badge-table").length;
                ...
                if (AppTable.isMergeTable) {
                    ...
                    if (AppTable.isSelectTableFirst) {
                        //first choose must not empty & select
                        if (tabelStatus != 1 && tabelStatus != 7 && tabelStatus != 6 && isMerged == 0) {
						...
						var isHasParent = $(this).data('parent-id') != "0";
						var isHasChild = $(".dine-in-order[data-parent-id="+$(this).data('table-id')+"]").length > 0;
						var isTableMerged = (isHasParent || isHasChild);
                        //second choose must empty & not merged
                        if (tabelStatus == 1 && !isTableMerged) {
                            AppTable.socket.emit('cm_set_table_used', {table_id: tableID});
                        } else {
                            AppTable.isSelectTableFirst = false;
                        }
                    }
            ...
            $('.new_order').unbind('click').bind('click', function(e) {
                ...
                var tableStatus = $("#tab_layout_"+table_id).data('table-status');
                if (tableStatus == 1 || tableStatus == 6) {
                    var request = $.ajax({
                        type: 'POST',
                        url: url,
                    ...
                    AppTable.alert("Meja sudah diapakai oleh konsumen lain, silahkan pilih meja lain.");
                }
            ...
            $(document).on('click', '.btn-change-floor', function(e) {
                if (AppNav.openCloseStatus != 1 || $(this).hasClass("disabled-floor")) return false;
                $('.alert-danger').hide();
                var floor = $('#floor_id');
            ...
            $("#delivery_cost_id").on('change', function(e) {
                AppTable.updateDeliveryOrderCalc();
            });
            $('.btn-change-table').on('click', function(e) {
                ...
                if (user_confirmation == undefined) user_confirmation = "";
                if (user_confirmation != "") {
					if($(this).find(".option-text").text().toLowerCase() != "batalkan"){
        ...
        closingOrder: function() {
            ...
            temp.each(function() {
                ...
                if (statusMenu == all_cooking_status[3]) {
                    countValidStatus += 1;
                    return;
                }
            ...
            if (countValidStatus == 0 && AppTable.diningType == 3) {
                AppTable.alert('Closing gagal, silahkan periksa kembali status pesanan anda.');
                return false;
            }

            AppTable.loadingOverlay.show();
            var orderId = $('#order_id').val();
            var url = $("#btn-close-order").attr('href');
            var request = $.ajax({
                type: 'POST',
                url: url,
                data: {
        ...
        initDelivery: function() {
            ...
            $("#delivery_cost_id").on('change', function() {
                ...
                is_percentage = $(this).find("option:selected").data("is_percentage");

                $("#delivery_cost").val(AppTable.formatDeliveryCost(delivery_cost, is_percentage));
            });
        ...
        disableEmptyTable: function(e) {
            ...
            if (e === true) {
                ...
                AppTable.tableParent.find(".dine-in-order").css("opacity", "1");
                AppTable.tableParent.find(".dine-in-order").css("cursor", "pointer");
                AppTable.tableParent.find(".label-triangle-empty").css("opacity", "0.3");
                AppTable.tableParent.find(".label-triangle-empty").css("cursor", "default");
                AppTable.tableParent.find(".label-rect-empty").css("opacity", "0.3");
                AppTable.tableParent.find(".label-rect-empty").css("cursor", "default");
                AppTable.tableParent.find(".label-circle-empty").css("opacity", "0.3");
                AppTable.tableParent.find(".label-circle-empty").css("cursor", "default");

                AppTable.tableParent.find(".dine-in-order").children(".badge-table").parent().css("opacity", "0.3");
                // AppTable.tableParent.find(".dine-in-order").children(".badge-table").parent().css("display", "none");
                AppTable.tableParent.find(".dine-in-order").children(".badge-table").parent().css("cursor", "default");

            } else {
                AppTable.tableParent.find(".dine-in-order").css("opacity", "0.3");
            ...
            AppTable.tableParent.find(".dine-in-order[data-table-status='7']").css({
                "opacity": "0.3",
                "cursor": "default"
            });
        },
        ...
        dineInProcess: function() {
                ...
                request.done(function(msg) {
                    if (msg != '') {
                        ...
                        if (parsedObject.status === true) {
                            ...
                            var delivery_cost_percentage = $("#delivery_cost_id").find("option:selected").data("is_percentage");
                            if (delivery_cost_percentage == 1) {
                                AppTable.updateDeliveryOrderCalc();
                            }
                        } else {
                            if (parsedObject.msg != '') {
            ...
            $('input:radio[name=pinter_checker]').change(function() {
                var request = $.ajax({
                    type: 'POST',
                    url: AppTable.baseUrl + 'table/update_printer_checker_default',
                    data: {
                        'printer_checker_selected': $(this).val()
                    }
                });
                request.done(function(msg) {
                    if (msg != '' && msg != '0') {
                ...
                request.fail(function(jqXHR, textStatus) {
                    if (textStatus == 'timeout') {
                        // AppTable.alert($('#server-timeout-message p').text());
                        console.log($('#server-timeout-message p').text());
                    }
                    window.location.reload(true);
                });
                request.always(function() {});
            ...
            $('#btn-process-order').unbind('click').bind('click', function(e) {
                e.preventDefault();
                var count_ord = $('.bill-table > tbody  > tr').length;
                if (count_ord == 0) {
            ...
            $(document).on('click', '#btn-close-order', function(e) {
                e.preventDefault();
                AppTable.confirm("Apakah orderan ini sudah selesai?", AppTable.closingOrder);
            });
        ...
        reservationProcess: function() {
            ...
            $('#btn-reserv-unlock').on('click', function(e) {
                ...
                if (user_confirmation == undefined) user_confirmation = "";
                if (user_confirmation != "") {
                    AppNav.showConfirmationPIN(user_confirmation, '#btn-reserv-unlock', "click", 1);
                    return;
                }
                var status = $(this).attr('data-status');
        ...
        /*
         * check floor prev/next
         * disable prev/next at first and last floor
         */
        checkPrevNextFloor: function() {
            var currentFloorId = parseInt($('#floor_id').val());
            var firstFloorId = parseInt($(".floor-list li:first-child > a").data("id"));
            var lastFloorId = parseInt($(".floor-list li:last-child > a").data("id"));

            $(".btn-change-floor").removeClass("disabled-floor");

            if (currentFloorId == firstFloorId) {
                $(".btn-prev.btn-change-floor").addClass("disabled-floor");
            }
            if (currentFloorId == lastFloorId) {
                $(".btn-next.btn-change-floor").addClass("disabled-floor");
            }
        },
         ...
         */
        formatDeliveryCost: function(deliveryCost, isPercentage) {
            deliveryCost = parseFloat(deliveryCost).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1.');
            deliveryCost = deliveryCost.slice(0, -3);

            if (isPercentage) {
                deliveryCost = deliveryCost + "%";
            } else {
                deliveryCost = "Rp " + deliveryCost;
            }
            return deliveryCost;
        },

        updateDeliveryOrderCalc:function(){
            var order_id            = $("#order_id").val();
            var delivery_cost_id    = $("#delivery_cost_id").find("option:selected").val();
            var delivery_cost       = $("#delivery_cost_id").find("option:selected").data("delivery_cost");
